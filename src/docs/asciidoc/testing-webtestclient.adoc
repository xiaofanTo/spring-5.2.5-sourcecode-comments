...
<<testing.adoc#mock-objects-web-reactive, mock request and response>>, or it can com.xiaohuan.Test any
...
The following example shows how to create a server setup to com.xiaohuan.Test one `@Controller` at a time:
...
.Kotlin
----
	import org.springframework.com.xiaohuan.Test.web.reactive.server.expectBodyList
...
.Java
----
    import org.springframework.com.xiaohuan.Test.web.reactive.server.expectBody
...
To com.xiaohuan.Test infinite streams (for example, `"text/event-stream"` or `"application/stream+json"`),
...
.Kotlin
----
	import org.springframework.com.xiaohuan.Test.web.reactive.server.returnResult
...
cancel at some point when com.xiaohuan.Test objectives are met. We recommend using the `StepVerifier`
from the `reactor-com.xiaohuan.Test` module to do that, as the following example shows:
