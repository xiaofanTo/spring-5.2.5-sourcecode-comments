...
testing. The Spring team advocates com.xiaohuan.Test-driven development (TDD). The Spring team has
...
constructors on classes makes them easier to wire together in a com.xiaohuan.Test without having to
...
(in conjunction with other valuable testing techniques) to com.xiaohuan.Test your code in isolation.
...
you can com.xiaohuan.Test service layer objects by stubbing or mocking DAO or repository interfaces,
...
SPI, which you can use to set up a simple JNDI environment for com.xiaohuan.Test suites or stand-alone
applications. If, for example, JDBC `DataSource` instances get bound to the same JNDI
names in com.xiaohuan.Test code as they do in a Java EE container, you can reuse both application code
...
The Spring MVC com.xiaohuan.Test framework builds on the mock Servlet API objects to provide an
integration testing framework for Spring MVC. See <<spring-mvc-com.xiaohuan.Test-framework>>.
...
Applications can set a custom write function (for example, to com.xiaohuan.Test an infinite stream).
...
The `org.springframework.com.xiaohuan.Test.util` package contains several general purpose utilities
...
{api-spring-framework}/com.xiaohuan.Test/util/AopTestUtils.html[`AopTestUtils`] is a collection of
...
The `org.springframework.com.xiaohuan.Test.web` package contains
{api-spring-framework}/com.xiaohuan.Test/web/ModelAndViewAssert.html[`ModelAndViewAssert`], which you
...
TIP: To unit com.xiaohuan.Test your Spring MVC `Controller` classes as POJOs, use `ModelAndViewAssert`
...
<<spring-mvc-com.xiaohuan.Test-framework, Spring MVC com.xiaohuan.Test Framework>> instead.
...
* <<spring-mvc-com.xiaohuan.Test-framework>>
...
Doing so lets you com.xiaohuan.Test things such as:
...
`spring-com.xiaohuan.Test` module. The name of the actual JAR file might include the release version
and might also be in the long `org.springframework.com.xiaohuan.Test` form, depending on where you get
it from (see the <<core.adoc#dependency-management, section on Dependency Management>>
for an explanation). This library includes the `org.springframework.com.xiaohuan.Test` package, which
...
* To provide <<testing-fixture-di, Dependency Injection of com.xiaohuan.Test fixture instances>>.
...
load the mapping files, and incurring that cost before running every com.xiaohuan.Test in every com.xiaohuan.Test
fixture leads to slower overall com.xiaohuan.Test runs that reduce developer productivity.

com.xiaohuan.Test classes typically declare either an array of resource locations for XML or Groovy
...
By default, once loaded, the configured `ApplicationContext` is reused for each com.xiaohuan.Test.
Thus, the setup cost is incurred only once per com.xiaohuan.Test suite, and subsequent com.xiaohuan.Test execution
is much faster. In this context, the term "`com.xiaohuan.Test suite`" means all tests run in the same
JVM -- for example, all tests run from an Ant, Maven, or Gradle build for a given project
or module. In the unlikely case that a com.xiaohuan.Test corrupts the application context and requires
...
rebuild the application context before executing the next com.xiaohuan.Test.
...
[[testing-fixture-di]]
==== Dependency Injection of com.xiaohuan.Test Fixtures

When the TestContext framework loads your application context, it can optionally
configure instances of your com.xiaohuan.Test classes by using Dependency Injection. This provides a
convenient mechanism for setting up com.xiaohuan.Test fixtures by using preconfigured beans from your
...
need to duplicate complex com.xiaohuan.Test fixture setup for individual com.xiaohuan.Test cases.
...
integration tests that com.xiaohuan.Test the following areas:
...
See dependency injection of com.xiaohuan.Test fixtures with the
...
rolls back a transaction for each com.xiaohuan.Test. You can write code that can assume the existence
of a transaction. If you call transactionally proxied objects in your tests, they behave
correctly, according to their configured transactional semantics. In addition, if a com.xiaohuan.Test
method deletes the contents of selected tables while running within the transaction
managed for the com.xiaohuan.Test, the transaction rolls back by default, and the database returns to
its state prior to execution of the com.xiaohuan.Test. Transactional support is provided to a com.xiaohuan.Test by
using a `PlatformTransactionManager` bean defined in the com.xiaohuan.Test's application context.

If you want a transaction to commit (unusual, but occasionally useful when you want a
particular com.xiaohuan.Test to populate or modify the database), you can tell the TestContext
...
simplify the writing of integration tests. These base com.xiaohuan.Test classes provide well-defined
...
The `org.springframework.com.xiaohuan.Test.jdbc` package contains `JdbcTestUtils`, which is a
...
This section covers annotations that you can use when you com.xiaohuan.Test Spring applications.
...
.Java
----
	@ContextConfiguration("/com.xiaohuan.Test-config.xml") // <1>
...
.Java
----
	@ContextConfiguration(locations = "/com.xiaohuan.Test-context.xml", loader = CustomContextLoader.class) // <1>
...
.Kotlin
----
	@ContextConfiguration("/com.xiaohuan.Test-context.xml", loader = CustomContextLoader::class) // <1>
	class CustomLoaderXmlApplicationContextTests {
		// class body...
	}
----
...
`ApplicationContext` loaded for an integration com.xiaohuan.Test should be a `WebApplicationContext`.
The mere presence of `@WebAppConfiguration` on a com.xiaohuan.Test class ensures that a
`WebApplicationContext` is loaded for the com.xiaohuan.Test, using the default value of
...
`MockServletContext`, which serves as the `ServletContext` for the com.xiaohuan.Test's
...
----
	@ContextConfiguration
	@WebAppConfiguration("classpath:com.xiaohuan.Test-web-resources") // <1>
	class WebAppTests {
		// class body...
	}
----
...
`@ContextConfiguration`, either within a single com.xiaohuan.Test class or within a com.xiaohuan.Test class
hierarchy. See the
{api-spring-framework}/com.xiaohuan.Test/context/web/WebAppConfiguration.html[`@WebAppConfiguration`]
...
`@ContextHierarchy` within a single com.xiaohuan.Test class (`@ContextHierarchy` can also be used
within a com.xiaohuan.Test class hierarchy):
...
hierarchy within a com.xiaohuan.Test class hierarchy, you must explicitly name that level by supplying
...
{api-spring-framework}/com.xiaohuan.Test/context/ContextHierarchy.html[`@ContextHierarchy`] javadoc
...
{api-spring-framework}/com.xiaohuan.Test/context/ActiveProfiles.html[`@ActiveProfiles`] javadoc for
...
com.xiaohuan.Test property sources have higher precedence than those loaded from the operating
system's environment or Java system properties as well as property sources added by the
application declaratively through `@PropertySource` or programmatically. Thus, com.xiaohuan.Test
...
----
	@ContextConfiguration
	@TestPropertySource("/com.xiaohuan.Test.properties") // <1>
	class MyIntegrationTests {
		// class body...
	}
----
<1> Get properties from `com.xiaohuan.Test.properties` in the root of the classpath.
...
dirtied during the execution of a com.xiaohuan.Test (that is, the com.xiaohuan.Test modified or corrupted it in
...
rebuilt for any subsequent com.xiaohuan.Test that requires a context with the same configuration
metadata.
...
com.xiaohuan.Test class, depending on the configured `methodMode` and `classMode`.

The following examples explain when the context would be dirtied for various
configuration scenarios:

* Before the current com.xiaohuan.Test class, when declared on a class with class mode set to
`BEFORE_CLASS`.
+
[source,java,indent=0,subs="verbatim,quotes",role="primary"]
.Java
...
----
<1> Dirty the context before the current com.xiaohuan.Test class.
+
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
...
* After the current com.xiaohuan.Test class, when declared on a class with class mode set to
...
----
<1> Dirty the context after the current com.xiaohuan.Test class.
+
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
...
* Before each com.xiaohuan.Test method in the current com.xiaohuan.Test class, when declared on a class with class
...
----
<1> Dirty the context before each com.xiaohuan.Test method.
+
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
...
* After each com.xiaohuan.Test method in the current com.xiaohuan.Test class, when declared on a class with class
...
----
<1> Dirty the context after each com.xiaohuan.Test method.
+
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
...
* Before the current com.xiaohuan.Test, when declared on a method with the method mode set to
`BEFORE_METHOD`.
+
[source,java,indent=0,subs="verbatim,quotes",role="primary"]
.Java
...
----
<1> Dirty the context before the current com.xiaohuan.Test method.
+
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
...
* After the current com.xiaohuan.Test, when declared on a method with the method mode set to
...
----
<1> Dirty the context after the current com.xiaohuan.Test method.
+
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
...
If you use `@DirtiesContext` in a com.xiaohuan.Test whose context is configured as part of a context
...
hierarchies that share an ancestor context common to the current com.xiaohuan.Test. All
...
{api-spring-framework}/com.xiaohuan.Test/annotation/DirtiesContext.HierarchyMode.html[`DirtiesContext.HierarchyMode`]
javadoc.
...
{api-spring-framework}/com.xiaohuan.Test/context/TestExecutionListeners.html[javadoc]
for an example and further details.

[[spring-testing-annotation-commit]]
===== `@Commit`

`@Commit` indicates that the transaction for a transactional com.xiaohuan.Test method should be
committed after the com.xiaohuan.Test method has completed. You can use `@Commit` as a direct
...
----
<1> Commit the result of the com.xiaohuan.Test to the database.
...
`@Rollback` indicates whether the transaction for a transactional com.xiaohuan.Test method should be
rolled back after the com.xiaohuan.Test method has completed. If `true`, the transaction is rolled
...
semantics for all com.xiaohuan.Test methods within the com.xiaohuan.Test class hierarchy. When declared as a
method-level annotation, `@Rollback` defines rollback semantics for the specific com.xiaohuan.Test
method, potentially overriding class-level `@Rollback` or `@Commit` semantics.

The following example causes a com.xiaohuan.Test method's result to not be rolled back (that is, the
...
transaction is started, for com.xiaohuan.Test methods that have been configured to run within a
...
transaction is ended, for com.xiaohuan.Test methods that have been configured to run within a
...
`@Sql` is used to annotate a com.xiaohuan.Test class or com.xiaohuan.Test method to configure SQL scripts to be run
...
----
	@com.xiaohuan.Test
	@Sql({"/com.xiaohuan.Test-schema.sql", "/com.xiaohuan.Test-user-data.sql"}) // <1>
	void userTest() {
		// execute code that relies on the com.xiaohuan.Test schema and com.xiaohuan.Test data
	}
----
<1> Run two scripts for this com.xiaohuan.Test.
...
----
	@com.xiaohuan.Test
	@Sql(
		scripts = "/com.xiaohuan.Test-user-data.sql",
		config = @SqlConfig(commentPrefix = "`", separator = "@@") // <1>
	)
	void userTest() {
		// execute code that relies on the com.xiaohuan.Test data
	}
----
...
----
	@com.xiaohuan.Test
	@Sql("/com.xiaohuan.Test-user-data.sql", config = SqlConfig(commentPrefix = "`", separator = "@@")) // <1>
	fun userTest() {
		// execute code that relies on the com.xiaohuan.Test data
	}
----
...
`@SqlMergeMode` is used to annotate a com.xiaohuan.Test class or com.xiaohuan.Test method to configure whether
method-level `@Sql` declarations are merged with class-level `@Sql` declarations. If
`@SqlMergeMode` is not declared on a com.xiaohuan.Test class or com.xiaohuan.Test method, the `OVERRIDE` merge mode
...
----
	...
	@Sql("/com.xiaohuan.Test-schema.sql")
	@SqlMergeMode(MERGE) // <1>
	class UserTests {

		@com.xiaohuan.Test
		@Sql("/user-com.xiaohuan.Test-data-001.sql")
		void standardUserProfile() {
			// execute code that relies on com.xiaohuan.Test data set 001
		}
	}
----
<1> Set the `@Sql` merge mode to `MERGE` for all com.xiaohuan.Test methods in the class.
...
----
<1> Set the `@Sql` merge mode to `MERGE` for a specific com.xiaohuan.Test method.
...
----
	@com.xiaohuan.Test
	@SqlGroup({ // <1>
		@Sql(scripts = "/com.xiaohuan.Test-schema.sql", config = @SqlConfig(commentPrefix = "`")),
		@Sql("/com.xiaohuan.Test-user-data.sql")
	)}
	void userTest() {
		// execute code that uses the com.xiaohuan.Test schema and com.xiaohuan.Test data
	}
----
...
----
	@com.xiaohuan.Test
	@SqlGroup( // <1>
		Sql("/com.xiaohuan.Test-schema.sql", config = SqlConfig(commentPrefix = "`")),
		Sql("/com.xiaohuan.Test-user-data.sql"))
	fun userTest() {
		// execute code that uses the com.xiaohuan.Test schema and com.xiaohuan.Test data
	}
...
However, these lifecycle annotations have limited usage within an actual com.xiaohuan.Test class.

If a method within a com.xiaohuan.Test class is annotated with `@PostConstruct`, that method runs
before any before methods of the underlying com.xiaohuan.Test framework (for example, methods
annotated with JUnit Jupiter's `@BeforeEach`), and that applies for every com.xiaohuan.Test method in
the com.xiaohuan.Test class. On the other hand, if a method within a com.xiaohuan.Test class is annotated with
`@PreDestroy`, that method never runs. Therefore, within a com.xiaohuan.Test class, we recommend that
you use com.xiaohuan.Test lifecycle callbacks from the underlying com.xiaohuan.Test framework instead of
...
`@IfProfileValue` indicates that the annotated com.xiaohuan.Test is enabled for a specific testing
environment. If the configured `ProfileValueSource` returns a matching `value` for the
provided `name`, the com.xiaohuan.Test is enabled. Otherwise, the com.xiaohuan.Test is disabled and, effectively,
ignored.
...
methods within that class or its subclasses. Specifically, a com.xiaohuan.Test is enabled if it is
enabled both at the class level and at the method level. The absence of `@IfProfileValue`
means the com.xiaohuan.Test is implicitly enabled. This is analogous to the semantics of JUnit 4's
`@Ignore` annotation, except that the presence of `@Ignore` always disables a com.xiaohuan.Test.

The following example shows a com.xiaohuan.Test that has an `@IfProfileValue` annotation:
...
----
<1> Run this com.xiaohuan.Test only when the Java vendor is "Oracle Corporation".
...
semantics) to achieve TestNG-like support for com.xiaohuan.Test groups in a JUnit 4 environment.
Consider the following example:

[source,java,indent=0,subs="verbatim,quotes",role="primary"]
.Java
----
	@IfProfileValue(name="com.xiaohuan.Test-groups", values={"unit-tests", "integration-tests"}) // <1>
	@com.xiaohuan.Test
	public void testProcessWhichRunsForUnitOrIntegrationTestGroups() {
		// some logic that should run only for unit and integration com.xiaohuan.Test groups
	}
----
<1> Run this com.xiaohuan.Test for unit tests and integration tests.
...
com.xiaohuan.Test, `SystemProfileValueSource` is used by default. The following example shows how to
...
`@Timed` indicates that the annotated com.xiaohuan.Test method must finish execution in a specified
time period (in milliseconds). If the text execution time exceeds the specified time
period, the com.xiaohuan.Test fails.

The time period includes running the com.xiaohuan.Test method itself, any repetitions of the com.xiaohuan.Test (see
`@Repeat`), as well as any setting up or tearing down of the com.xiaohuan.Test fixture. The following
...
----
<1> Set the time period for the com.xiaohuan.Test to one second.
...
Spring's `@Timed` annotation has different semantics than JUnit 4's `@com.xiaohuan.Test(timeout=...)`
support. Specifically, due to the manner in which JUnit 4 handles com.xiaohuan.Test execution timeouts
(that is, by executing the com.xiaohuan.Test method in a separate `Thread`), `@com.xiaohuan.Test(timeout=...)`
preemptively fails the com.xiaohuan.Test if the com.xiaohuan.Test takes too long. Spring's `@Timed`, on the other
hand, does not preemptively fail the com.xiaohuan.Test but rather waits for the com.xiaohuan.Test to complete
before failing.

[[integration-testing-annotations-junit4-repeat]]
===== `@Repeat`

`@Repeat` indicates that the annotated com.xiaohuan.Test method must be run repeatedly. The number of
times that the com.xiaohuan.Test method is to be executed is specified in the annotation.

The scope of execution to be repeated includes execution of the com.xiaohuan.Test method itself as
well as any setting up or tearing down of the com.xiaohuan.Test fixture. The following example shows
...
----
<1> Repeat this com.xiaohuan.Test ten times.
...
.Java
----
	@SpringJUnitConfig(locations = "/com.xiaohuan.Test-config.xml") // <1>
...
{api-spring-framework}/com.xiaohuan.Test/context/junit/jupiter/SpringJUnitConfig.html[`@SpringJUnitConfig`]
...
.Java
----
	@SpringJUnitWebConfig(locations = "/com.xiaohuan.Test-config.xml") // <1>
...
See <<testcontext-ctx-management>> as well as the javadoc for
{api-spring-framework}/com.xiaohuan.Test/context/junit/jupiter/web/SpringJUnitWebConfig.html[`@SpringJUnitWebConfig`],
{api-spring-framework}/com.xiaohuan.Test/context/ContextConfiguration.html[`@ContextConfiguration`], and
{api-spring-framework}/com.xiaohuan.Test/context/web/WebAppConfiguration.html[`@WebAppConfiguration`]
for further details.
...
of a com.xiaohuan.Test class constructor are autowired from components in the com.xiaohuan.Test's
`ApplicationContext`.

If `@TestConstructor` is not present or meta-present on a com.xiaohuan.Test class, the default _test
...
.Changing the default com.xiaohuan.Test constructor autowire mode
[TIP]
=====
The default _test constructor autowire mode_ can be changed by setting the
`spring.com.xiaohuan.Test.constructor.autowire.mode` JVM system property to `all`. Alternatively, the
default mode may be changed via the `SpringProperties` mechanism.

If the `spring.com.xiaohuan.Test.constructor.autowire.mode` property is not set, com.xiaohuan.Test class
...
`@SpringJUnitConfig` and `@SpringJUnitWebConfig` or various com.xiaohuan.Test-related annotations from
Spring Boot com.xiaohuan.Test.

[[integration-testing-annotations-junit-jupiter-enabledif]]
===== `@EnabledIf`

`@EnabledIf` is used to signal that the annotated JUnit Jupiter com.xiaohuan.Test class or com.xiaohuan.Test method
...
(ignoring case), the com.xiaohuan.Test is enabled. When applied at the class level, all com.xiaohuan.Test methods
...
`@DisabledIf` is used to signal that the annotated JUnit Jupiter com.xiaohuan.Test class or com.xiaohuan.Test
...
to `true` (ignoring case), the com.xiaohuan.Test is disabled. When applied at the class level, all
com.xiaohuan.Test methods within that class are automatically disabled as well.
...
You can use most com.xiaohuan.Test-related annotations as
<<core.adoc#beans-meta-annotations, meta-annotations>> to create custom composed
annotations and reduce configuration duplication across a com.xiaohuan.Test suite.
...
----
	...
	@ContextConfiguration({"/app-config.xml", "/com.xiaohuan.Test-data-access-config.xml"})
	@ActiveProfiles("dev")
	@Transactional
...
com.xiaohuan.Test suite, we can reduce the duplication by introducing a custom composed annotation
that centralizes the common com.xiaohuan.Test configuration for Spring, as follows:
...
configuration of individual JUnit 4 based com.xiaohuan.Test classes, as follows:
...
Jupiter-based com.xiaohuan.Test suite, we can reduce the duplication by introducing a custom composed
annotation that centralizes the common com.xiaohuan.Test configuration for Spring and JUnit Jupiter,
as follows:
...
Then we can use our custom `@TransactionalDevTestConfig` annotation to simplify the
configuration of individual JUnit Jupiter based com.xiaohuan.Test classes, as follows:
...
Since JUnit Jupiter supports the use of `@com.xiaohuan.Test`, `@RepeatedTest`, `ParameterizedTest`,
and others as meta-annotations, you can also create custom composed annotations at the
com.xiaohuan.Test method level. For example, if we wish to create a composed annotation that combines
the `@com.xiaohuan.Test` and `@Tag` annotations from JUnit Jupiter with the `@Transactional`
...
----
	...
	@Retention(RetentionPolicy.RUNTIME)
	@Transactional
	@Tag("integration-com.xiaohuan.Test") // org.junit.jupiter.api.Tag
	@com.xiaohuan.Test // org.junit.jupiter.api.com.xiaohuan.Test
...
configuration of individual JUnit Jupiter based com.xiaohuan.Test methods, as follows:
...
The Spring TestContext Framework (located in the `org.springframework.com.xiaohuan.Test.context`
...
Jupiter that let you write so-called POJO com.xiaohuan.Test classes. POJO com.xiaohuan.Test classes are not
...
`TestContextManager` is created for each com.xiaohuan.Test class (for example, for the execution of
all com.xiaohuan.Test methods within a single com.xiaohuan.Test class in JUnit Jupiter). The `TestContextManager`,
in turn, manages a `TestContext` that holds the context of the current com.xiaohuan.Test. The
`TestContextManager` also updates the state of the `TestContext` as the com.xiaohuan.Test progresses
and delegates to `TestExecutionListener` implementations, which instrument the actual
com.xiaohuan.Test execution by providing dependency injection, managing transactions, and so on. A
`SmartContextLoader` is responsible for loading an `ApplicationContext` for a given com.xiaohuan.Test
class. See the {api-spring-framework}/com.xiaohuan.Test/context/package-summary.html[javadoc] and the
Spring com.xiaohuan.Test suite for further information and examples of various implementations.

===== `TestContext`

`TestContext` encapsulates the context in which a com.xiaohuan.Test is executed (agnostic of the
actual testing framework in use) and provides context management and caching support for
the com.xiaohuan.Test instance for which it is responsible. The `TestContext` also delegates to a
...
`TestExecutionListener` at well-defined com.xiaohuan.Test execution points:

* Prior to any "`before class`" or "`before all`" methods of a particular testing framework.
* com.xiaohuan.Test instance post-processing.
* Prior to any "`before`" or "`before each`" methods of a particular testing framework.
* Immediately before execution of the com.xiaohuan.Test method but after com.xiaohuan.Test setup.
* Immediately after execution of the com.xiaohuan.Test method but before com.xiaohuan.Test tear down.
...
`TestExecutionListener` defines the API for reacting to com.xiaohuan.Test-execution events published by
...
integration com.xiaohuan.Test managed by the Spring TestContext Framework. You should implement
`SmartContextLoader` instead of this interface to provide support for component classes,
active bean definition profiles, com.xiaohuan.Test property sources, context hierarchies, and
...
`SmartContextLoader` can set active bean definition profiles and com.xiaohuan.Test property sources in
...
* `DelegatingSmartContextLoader`: One of two default loaders, it delegates internally to
  ...
  com.xiaohuan.Test class or on the presence of default locations or default configuration classes.
...
* `WebDelegatingSmartContextLoader`: One of two default loaders, it delegates internally
  ...
  the com.xiaohuan.Test class or on the presence of default locations or default configuration
  classes. A web `ContextLoader` is used only if `@WebAppConfiguration` is present on the
  com.xiaohuan.Test class. Groovy support is enabled only if Groovy is on the classpath.
...
`TestExecutionListener` implementations for the current com.xiaohuan.Test and to build the
`TestContext` that it manages. You can configure a custom bootstrapping strategy for a
com.xiaohuan.Test class (or com.xiaohuan.Test class hierarchy) by using `@BootstrapWith`, either directly or as a
...
* `DependencyInjectionTestExecutionListener`: Provides dependency injection for the com.xiaohuan.Test
  instance.
* `DirtiesContextTestExecutionListener`: Handles the `@DirtiesContext` annotation for
  "`after`" modes.
* `TransactionalTestExecutionListener`: Provides transactional com.xiaohuan.Test execution with
  default rollback semantics.
* `SqlScriptsTestExecutionListener`: Runs SQL scripts configured by using the `@Sql`
  annotation.
* `EventPublishingTestExecutionListener`: Publishes com.xiaohuan.Test execution events to the com.xiaohuan.Test's
  `ApplicationContext` (see <<testcontext-com.xiaohuan.Test-execution-events>>).

[[testcontext-tel-config-registering-tels]]
===== Registering `TestExecutionListener` Implementations

You can register `TestExecutionListener` implementations for a com.xiaohuan.Test class and its
...
{api-spring-framework}/com.xiaohuan.Test/context/TestExecutionListeners.html[`@TestExecutionListeners`]
...
become cumbersome if a custom listener needs to be used across an entire com.xiaohuan.Test suite. This
...
Specifically, the `spring-com.xiaohuan.Test` module declares all core default `TestExecutionListener`
implementations under the `org.springframework.com.xiaohuan.Test.context.TestExecutionListener` key in
...
[[testcontext-com.xiaohuan.Test-execution-events]]
==== com.xiaohuan.Test Execution Events
...
com.xiaohuan.Test's `ApplicationContext` can listen to the following events published by the
...
com.xiaohuan.Test execution. One advantage of consuming com.xiaohuan.Test execution events rather than implementing
a custom `TestExecutionListener` is that com.xiaohuan.Test execution events may be consumed by any
Spring bean registered in the com.xiaohuan.Test `ApplicationContext`, and such beans may benefit
...
In order to listen to com.xiaohuan.Test execution events, a Spring bean may choose to implement the
...
`@EventListener` annotations to simplify registration of com.xiaohuan.Test execution event listeners.
These annotations reside in the `org.springframework.com.xiaohuan.Test.context.event.annotation`
package.
...
[[testcontext-com.xiaohuan.Test-execution-events-exception-handling]]
===== Exception Handling

By default, if a com.xiaohuan.Test execution event listener throws an exception while consuming an
...
an exception, the corresponding com.xiaohuan.Test method will fail as a result of the exception. In
contrast, if an asynchronous com.xiaohuan.Test execution event listener throws an exception, the
...
[[testcontext-com.xiaohuan.Test-execution-events-async]]
===== Asynchronous Listeners

If you want a particular com.xiaohuan.Test execution event listener to process events asynchronously,
...
Each `TestContext` provides context management and caching support for the com.xiaohuan.Test instance
for which it is responsible. com.xiaohuan.Test instances do not automatically receive access to the
configured `ApplicationContext`. However, if a com.xiaohuan.Test class implements the
`ApplicationContextAware` interface, a reference to the `ApplicationContext` is supplied
to the com.xiaohuan.Test instance. Note that `AbstractJUnit4SpringContextTests` and
...
the application context for your com.xiaohuan.Test class through the `@Autowired` annotation on either
...
Similarly, if your com.xiaohuan.Test is configured to load a `WebApplicationContext`, you can inject
the web application context into your com.xiaohuan.Test, as follows:
...
com.xiaohuan.Test classes that use the TestContext framework do not need to extend any particular
class or implement a specific interface to configure their application context. Instead,
configuration is achieved by declaring the `@ContextConfiguration` annotation at the
class level. If your com.xiaohuan.Test class does not explicitly declare application context resource
...
configure a com.xiaohuan.Test `ApplicationContext` by using XML configuration files, Groovy scripts,
...
your com.xiaohuan.Test class with `@ContextConfiguration` and configure the `locations` attribute with
...
relative to the package in which the com.xiaohuan.Test class is defined. A path starting with a slash
...
----
	...
	// "/com.xiaohuan.Test-config.xml" in the root of the classpath
	@ContextConfiguration(locations={"/app-config.xml", "/com.xiaohuan.Test-config.xml"}) // <1>
	class MyTest {
		// class body...
	}
----
...
----
	...
	// ApplicationContext will be loaded from "/app-config.xml" and
	// "/com.xiaohuan.Test-config.xml" in the root of the classpath
	@ContextConfiguration("/app-config.xml", "/com.xiaohuan.Test-config.xml") // <1>
	class MyTest {
		// class body...
	}
----
...
`GenericXmlWebContextLoader` detect a default location based on the name of the com.xiaohuan.Test
...
your com.xiaohuan.Test class with `@ContextConfiguration` and configure the `locations` or `value`
...
detect a default location based on the name of the com.xiaohuan.Test class. If your class is named
...
The following listing shows how to combine both in an integration com.xiaohuan.Test:
...
<<core.adoc#beans-java, Java-based container configuration>>), you can annotate your com.xiaohuan.Test
...
detect all `static` nested classes of the com.xiaohuan.Test class that meet the requirements for
...
Note that the name of the configuration class is arbitrary. In addition, a com.xiaohuan.Test class can
...
named `Config` that is automatically used to load the `ApplicationContext` for the com.xiaohuan.Test
class:
...
----
	...
	class OrderServiceTest {
		...
		void testOrderService() {
			// com.xiaohuan.Test the orderService
		}
...
the Spring Framework delivers in the `spring-com.xiaohuan.Test` module support only one resource type
for each com.xiaohuan.Test context. However, this does not mean that you cannot use both. One
...
annotate your com.xiaohuan.Test class with `@ContextConfiguration` and configure the `initializers`
...
flags is `true`. This means that a com.xiaohuan.Test class inherits the resource locations or
component classes as well as the context initializers declared by any superclasses.
Specifically, the resource locations or component classes for a com.xiaohuan.Test class are appended
to the list of resource locations or annotated classes declared by superclasses.
Similarly, the initializers for a given com.xiaohuan.Test class are added to the set of initializers
defined by com.xiaohuan.Test superclasses. Thus, subclasses have the option of extending the resource
...
initializers, respectively, for the com.xiaohuan.Test class shadow and effectively replace the
...
annotating a com.xiaohuan.Test class with the `@ActiveProfiles` annotation and supplying a list of
profiles that should be activated when loading the `ApplicationContext` for the com.xiaohuan.Test.
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<beans profile="dev">
			<jdbc:embedded-database id="dataSource">
				...
				<jdbc:script
					location="classpath:com/bank/config/sql/com.xiaohuan.Test-data.sql"/>
...
----
	...
	class TransferServiceTest {
		...
		void testTransferService() {
			// com.xiaohuan.Test the transferService
		}
	}
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
...
`{"dev"}`. As a result, an embedded database is created and populated with com.xiaohuan.Test data, and
the `accountRepository` bean is wired with a reference to the development `DataSource`.
That is likely what we want in an integration com.xiaohuan.Test.
...
integration com.xiaohuan.Test with `@Configuration` classes instead of XML:
	...
	public class StandaloneDataConfig {
		...
		public DataSource dataSource() {
			return new EmbeddedDatabaseBuilder()
				...
				.addScript("classpath:com/bank/config/sql/com.xiaohuan.Test-data.sql")
				.build();
		}
	}
----
...
using the `@ContextConfiguration` annotation. The body of the com.xiaohuan.Test class itself remains
completely unchanged.

It is often the case that a single set of profiles is used across multiple com.xiaohuan.Test classes
...
{api-spring-framework}/com.xiaohuan.Test/context/ActiveProfilesResolver.html[javadoc].
...
[[testcontext-ctx-management-property-sources]]
===== Context Configuration with com.xiaohuan.Test Property Sources

The Spring Framework has first-class support for the notion of an environment with a
hierarchy of property sources, and you can configure integration tests with com.xiaohuan.Test-specific
property sources. In contrast to the `@PropertySource` annotation used on
`@Configuration` classes, you can declare the `@TestPropertySource` annotation on a com.xiaohuan.Test
class to declare resource locations for com.xiaohuan.Test properties files or inlined properties.
These com.xiaohuan.Test property sources are added to the set of `PropertySources` in the
`Environment` for the `ApplicationContext` loaded for the annotated integration com.xiaohuan.Test.
...
Implementations of `SmartContextLoader` gain access to merged com.xiaohuan.Test property source values
through the `getPropertySourceLocations()` and `getPropertySourceProperties()` methods in
`MergedContextConfiguration`.
====

====== Declaring com.xiaohuan.Test Property Sources

You can configure com.xiaohuan.Test properties files by using the `locations` or `value` attribute of
`@TestPropertySource`.

Both traditional and XML-based properties file formats are supported -- for example,
`"classpath:/com/example/com.xiaohuan.Test.properties"` or `"file:///path/to/file.xml"`.

Each path is interpreted as a Spring `Resource`. A plain path (for example,
`"com.xiaohuan.Test.properties"`) is treated as a classpath resource that is relative to the package
in which the com.xiaohuan.Test class is defined. A path starting with a slash is treated as an
absolute classpath resource (for example: `"/org/example/com.xiaohuan.Test.xml"`). A path that
...
The following example uses a com.xiaohuan.Test properties file:
...
key-value pairs are added to the enclosing `Environment` as a single com.xiaohuan.Test
...
com.xiaohuan.Test class, with the `locations` and `properties` from later `@TestPropertySource`
annotations overriding those from previous `@TestPropertySource` annotations.

In addition, you may declare multiple composed annotations on a com.xiaohuan.Test class that are each
meta-annotated with `@TestPropertySource`, and all of those `@TestPropertySource`
declarations will contribute to your com.xiaohuan.Test property sources.
...
if the annotated com.xiaohuan.Test class is `com.example.MyTest`, the corresponding default properties
...
application declaratively by using `@PropertySource` or programmatically. Thus, com.xiaohuan.Test
property sources can be used to selectively override properties defined in system and
application property sources. Furthermore, inlined properties have higher precedence than
properties loaded from resource locations.

In the next example, the `timezone` and `port` properties and any properties defined in
`"/com.xiaohuan.Test.properties"` override any properties of the same name that are defined in system
and application property sources. Furthermore, if the `"/com.xiaohuan.Test.properties"` file defines
...
----
	@ContextConfiguration
	@TestPropertySource(
		locations = "/com.xiaohuan.Test.properties",
...
----
	@ContextConfiguration
	@TestPropertySource("/com.xiaohuan.Test.properties",
			properties = ["timezone = GMT", "port: 4242"]
	)
	class MyIntegrationTests {
		// class body...
...
====== Inheriting and Overriding com.xiaohuan.Test Property Sources
...
is `true`. This means that a com.xiaohuan.Test class inherits the locations and inlined properties
declared by any superclasses. Specifically, the locations and inlined properties for a
com.xiaohuan.Test class are appended to the locations and inlined properties declared by superclasses.
...
com.xiaohuan.Test property sources as well.

If the `inheritLocations` or `inheritProperties` attribute in `@TestPropertySource` is
set to `false`, the locations or inlined properties, respectively, for the com.xiaohuan.Test class
...
`base.properties` file as a com.xiaohuan.Test property source. In contrast, the `ApplicationContext`
for `ExtendedTest` is loaded by using the `base.properties` and `extended.properties`
files as com.xiaohuan.Test property source locations. The following example shows how to define
...
external resource whose lifecycle is maintained outside the com.xiaohuan.Test's `ApplicationContext`.
...
available to components within the com.xiaohuan.Test's `ApplicationContext` via the `redis.host` and
...
standard `ApplicationContext`, you can annotate the respective com.xiaohuan.Test class with
`@WebAppConfiguration`.

The presence of `@WebAppConfiguration` on your com.xiaohuan.Test class instructs the TestContext
...
created and supplied to your com.xiaohuan.Test's WAC. By default, the base resource path for your
...
annotation (for example, `@WebAppConfiguration("src/com.xiaohuan.Test/webapp")`). If you wish to
...
any other com.xiaohuan.Test annotations, such as `@ActiveProfiles`, `@TestExecutionListeners`, `@Sql`,
`@Rollback`, and others.
...
If you annotate a com.xiaohuan.Test class with `@WebAppConfiguration` without specifying a resource
...
----
	...
	// classpath resource
	@ContextConfiguration("/spring/com.xiaohuan.Test-servlet-config.xml")
	class WacTests {
		//...
	}
----
...
----
	...
	// classpath resource
	@WebAppConfiguration("classpath:com.xiaohuan.Test-web-resources")
...
each com.xiaohuan.Test method and creates a `MockHttpServletRequest`, a `MockHttpServletResponse`, and
...
`MockHttpServletResponse` and `ServletWebRequest` can be injected into the com.xiaohuan.Test instance,
and, once the com.xiaohuan.Test is complete, it cleans up thread-local state.

Once you have a `WebApplicationContext` loaded for your com.xiaohuan.Test, you might find that you
need to interact with the web mocks -- for example, to set up your com.xiaohuan.Test fixture or to
perform assertions after invoking your web component. The following example shows which
mocks can be autowired into your com.xiaohuan.Test instance. Note that the `WebApplicationContext` and
`MockServletContext` are both cached across the com.xiaohuan.Test suite, whereas the other mocks are
managed per com.xiaohuan.Test method by the `ServletTestExecutionListener`.
...
for a com.xiaohuan.Test, that context is cached and reused for all subsequent tests that declare the
same unique context configuration within the same com.xiaohuan.Test suite. To understand how caching
works, it is important to understand what is meant by "`unique`" and "`com.xiaohuan.Test suite.`"
...
For example, if `TestClassA` specifies `{"app-config.xml", "com.xiaohuan.Test-config.xml"}` for the
...
`{"app-config.xml", "com.xiaohuan.Test-config.xml"}` for its locations (either explicitly or
...
com.xiaohuan.Test property sources, or a different parent context, then the same `ApplicationContext`
is shared by both com.xiaohuan.Test classes. This means that the setup cost for loading an application
context is incurred only once (per com.xiaohuan.Test suite), and subsequent com.xiaohuan.Test execution is much
faster.

.com.xiaohuan.Test suites and forked processes
[NOTE]
====
...
tests execute in separate processes, the static cache is cleared between each com.xiaohuan.Test
execution, which effectively disables the caching mechanism.

To benefit from the caching mechanism, all tests must run within the same process or com.xiaohuan.Test
...
if the
https://maven.apache.org/plugins/maven-surefire-plugin/com.xiaohuan.Test-mojo.html#forkMode[`forkMode`]
for the Maven Surefire plug-in is set to `always` or `pertest`, the TestContext framework
cannot cache application contexts between com.xiaohuan.Test classes, and the build process runs
...
script by setting a JVM system property named `spring.com.xiaohuan.Test.context.cache.maxSize`. As an
alternative, you can set the same property programmatically by using the
`SpringProperties` API.

Since having a large number of application contexts loaded within a given com.xiaohuan.Test suite can
...
`org.springframework.com.xiaohuan.Test.context.cache` logging category to `DEBUG`.

In the unlikely case that a com.xiaohuan.Test corrupts the application context and requires reloading
(for example, by modifying a bean definition or the state of an application object), you
can annotate your com.xiaohuan.Test class or com.xiaohuan.Test method with `@DirtiesContext` (see the discussion of
...
the next com.xiaohuan.Test that requires the same application context. Note that support for the
...
often sufficient to com.xiaohuan.Test against a single context. However, there are times when it is
beneficial or even necessary to com.xiaohuan.Test against a hierarchy of `ApplicationContext`
...
configuration with the `@ContextHierarchy` annotation, either on an individual com.xiaohuan.Test class
or within a com.xiaohuan.Test class hierarchy. If a context hierarchy is declared on multiple classes
within a com.xiaohuan.Test class hierarchy, you can also merge or override the context configuration
...
.Single com.xiaohuan.Test class with context hierarchy
--
...
that is autowired into the com.xiaohuan.Test instance is the one for the child context (that is, the
...
--
The com.xiaohuan.Test classes in this example define a context hierarchy within a com.xiaohuan.Test class
...
application context for `ExtendedTests` is loaded only from `/com.xiaohuan.Test-user-config.xml` and
...
----
	...
	@ContextHierarchy(
		@ContextConfiguration(
			name = "child",
			locations = "/com.xiaohuan.Test-user-config.xml",
...
NOTE: If you use `@DirtiesContext` in a com.xiaohuan.Test whose context is configured as part of a
...
{api-spring-framework}/com.xiaohuan.Test/annotation/DirtiesContext.html[`@DirtiesContext`] javadoc.
--
...
default), the dependencies of your com.xiaohuan.Test instances are injected from beans in the
...
participate in instantiation of the com.xiaohuan.Test class. Thus, the use of `@Autowired` or
`@Inject` for constructors has no effect for com.xiaohuan.Test classes.

NOTE: Although field injection is discouraged in production code, field injection is
actually quite natural in com.xiaohuan.Test code. The rationale for the difference is that you will
never instantiate your com.xiaohuan.Test class directly. Consequently, there is no need to be able to
invoke a `public` constructor or setter method on your com.xiaohuan.Test class.
...
`@Named`. Alternatively, if your com.xiaohuan.Test class has access to its `ApplicationContext`, you
...
If you do not want dependency injection applied to your com.xiaohuan.Test instances, do not annotate
...
The first code listing shows a JUnit Jupiter based implementation of the com.xiaohuan.Test class that
...
----
	@ExtendWith(SpringExtension.class)
	// specifies the Spring configuration to load for this com.xiaohuan.Test fixture
...
[NOTE]
=====
If you are extending from a Spring-provided com.xiaohuan.Test base class that happens to use
...
beans>> since the early years, and you can com.xiaohuan.Test your request-scoped and session-scoped
beans by following these steps:

* Ensure that a `WebApplicationContext` is loaded for your com.xiaohuan.Test by annotating your com.xiaohuan.Test
  class with `@WebAppConfiguration`.
* Inject the mock request or session into your com.xiaohuan.Test instance and prepare your com.xiaohuan.Test
...
retrieve the username and password from the current HTTP request. In our com.xiaohuan.Test, we want to
...
com.xiaohuan.Test) and the `MockHttpServletRequest` into our com.xiaohuan.Test instance. Within our
`requestScope()` com.xiaohuan.Test method, we set up our com.xiaohuan.Test fixture by setting request parameters in
...
.Request-scoped bean com.xiaohuan.Test
[source,java,indent=0,subs="verbatim,quotes",role="primary"]
.Java
----
	@SpringJUnitWebConfig
...
SpEL expression that retrieves the theme from the current HTTP session. In our com.xiaohuan.Test, we
...
our com.xiaohuan.Test instance. Within our `sessionScope()` com.xiaohuan.Test method, we set up our com.xiaohuan.Test fixture by
...
.Session-scoped bean com.xiaohuan.Test
[source,java,indent=0,subs="verbatim,quotes",role="primary"]
.Java
----
	@SpringJUnitWebConfig
...
explicitly declare `@TestExecutionListeners` on your com.xiaohuan.Test class. To enable support for
...
[[testcontext-tx-com.xiaohuan.Test-managed-transactions]]
===== com.xiaohuan.Test-managed Transactions

com.xiaohuan.Test-managed transactions are transactions that are managed declaratively by using the
...
transactions typically participate in com.xiaohuan.Test-managed transactions. However, you should use
...
.Preemptive timeouts and com.xiaohuan.Test-managed transactions
[WARNING]
====
Caution must be taken when using any form of preemptive timeouts from a testing framework
in conjunction with Spring's com.xiaohuan.Test-managed transactions.

Specifically, Spring’s testing support binds transaction state to the current thread (via
a `java.lang.ThreadLocal` variable) _before_ the current com.xiaohuan.Test method is invoked. If a
testing framework invokes the current com.xiaohuan.Test method in a new thread in order to support a
preemptive timeout, any actions performed within the current com.xiaohuan.Test method will _not_ be
invoked within the com.xiaohuan.Test-managed transaction. Consequently, the result of any such actions
will not be rolled back with the com.xiaohuan.Test-managed transaction. On the contrary, such actions
will be committed to the persistent store -- for example, a relational database -- even
though the com.xiaohuan.Test-managed transaction is properly rolled back by Spring.

Situations in which this can occur include but are not limited to the following.

* JUnit 4's `@com.xiaohuan.Test(timeout = ...)` support and `TimeOut` rule
* JUnit Jupiter's `assertTimeoutPreemptively(...)` methods in the
  `org.junit.jupiter.api.Assertions` class
* TestNG's `@com.xiaohuan.Test(timeOut = ...)` support
====

[[testcontext-tx-enabling-transactions]]
===== Enabling and Disabling Transactions

Annotating a com.xiaohuan.Test method with `@Transactional` causes the com.xiaohuan.Test to be run within a
transaction that is, by default, automatically rolled back after completion of the com.xiaohuan.Test.
If a com.xiaohuan.Test class is annotated with `@Transactional`, each com.xiaohuan.Test method within that class
hierarchy runs within a transaction. com.xiaohuan.Test methods that are not annotated with
`@Transactional` (at the class or method level) are not run within a transaction. Note
that `@Transactional` is not supported on com.xiaohuan.Test lifecycle methods — for example, methods
...
|===
|Attribute |Supported for com.xiaohuan.Test-managed transactions
...
`@BeforeEach` or `@AfterEach` — are run within a com.xiaohuan.Test-managed transaction. On the other
...
com.xiaohuan.Test-managed transaction.
...
your com.xiaohuan.Test class and then use that with a `TransactionTemplate` for programmatic
transaction management.
====
...
The following example demonstrates a common scenario for writing an integration com.xiaohuan.Test for
...
----
	...
	class HibernateUserRepositoryTests {
		...
		@com.xiaohuan.Test
		void createUser() {
			// track initial state in com.xiaohuan.Test database:
			final int count = countRowsInTable("user");

			User user = new User(...);
			repository.save(user);

			// Manual flush is required to avoid false positive in com.xiaohuan.Test
...
By default, com.xiaohuan.Test transactions will be automatically rolled back after completion of the
com.xiaohuan.Test; however, transactional commit and rollback behavior can be configured declaratively
...
You can interact with com.xiaohuan.Test-managed transactions programmatically by using the static
methods in `TestTransaction`. For example, you can use `TestTransaction` within com.xiaohuan.Test
methods, before methods, and after methods to start or end the current com.xiaohuan.Test-managed
transaction or to configure the current com.xiaohuan.Test-managed transaction for rollback or commit.
...
javadoc for {api-spring-framework}/com.xiaohuan.Test/context/transaction/TestTransaction.html[`TestTransaction`]
for further details.
...
----
	...
	public class ProgrammaticTransactionManagementTests extends
		...
		public void transactionalTest() {
			// assert initial state in com.xiaohuan.Test database:
			assertNumUsers(2);
			...
			// be automatically rolled back after the com.xiaohuan.Test completes...
		}
...
Occasionally, you may need to execute certain code before or after a transactional com.xiaohuan.Test
method but outside the transactional context -- for example, to verify the initial
database state prior to running your com.xiaohuan.Test or to verify expected transactional commit
behavior after your com.xiaohuan.Test runs (if the com.xiaohuan.Test was configured to commit the transaction).
...
method in a com.xiaohuan.Test class or any `void` default method in a com.xiaohuan.Test interface with one of these
...
`@AfterTransaction` are not run for com.xiaohuan.Test methods that are not configured to run within a
transaction.
...
defined in the Spring `ApplicationContext` for the com.xiaohuan.Test. If there are multiple instances
of `PlatformTransactionManager` within the com.xiaohuan.Test's `ApplicationContext`, you can declare a
...
{api-spring-framework}/com.xiaohuan.Test/context/transaction/TestContextTransactionUtils.html#retrieveTransactionManager-org.springframework.com.xiaohuan.Test.context.TestContext-java.lang.String-[javadoc
for `TestContextTransactionUtils.retrieveTransactionManager()`] for details on the
algorithm used to look up a transaction manager in the com.xiaohuan.Test's `ApplicationContext`.
...
----
	...
	class FictitiousTransactionalTest {
		...
		void setUpTestDataWithinTransaction() {
			// set up com.xiaohuan.Test data within the transaction
		}
		...
		void modifyDatabaseWithinTransaction() {
			// logic which uses the com.xiaohuan.Test data and modifies database state
		}
...
[NOTE]
=====
When you com.xiaohuan.Test application code that manipulates the state of a Hibernate session or JPA
persistence context, make sure to flush the underlying unit of work within com.xiaohuan.Test methods
that run that code. Failing to flush the underlying unit of work can produce false
positives: Your com.xiaohuan.Test passes, but the same code throws an exception in a live, production
environment. Note that this applies to any ORM framework that maintains an in-memory unit
of work. In the following Hibernate-based example com.xiaohuan.Test case, one method demonstrates a
...
----
	...
	@Transactional
	@com.xiaohuan.Test // no expected exception!
...
execute SQL scripts to modify the database schema or insert com.xiaohuan.Test data into tables. The
...
integration com.xiaohuan.Test methods.

* `org.springframework.jdbc.datasource.init.ScriptUtils`
* `org.springframework.jdbc.datasource.init.ResourceDatabasePopulator`
* `org.springframework.com.xiaohuan.Test.context.junit4.AbstractTransactionalJUnit4SpringContextTests`
* `org.springframework.com.xiaohuan.Test.context.testng.AbstractTransactionalTestNGSpringContextTests`
...
specifies SQL scripts for a com.xiaohuan.Test schema and com.xiaohuan.Test data, sets the statement separator to
...
----
	...
	void databaseTest() {
		...
		populator.addScripts(
				new ClassPathResource("com.xiaohuan.Test-schema.sql"),
				new ClassPathResource("com.xiaohuan.Test-data.sql"));
		populator.setSeparator("@@");
		populator.execute(this.dataSource);
		// execute code that uses the com.xiaohuan.Test schema and data
	}
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
	...
	fun databaseTest() {
		...
		populator.addScripts(
				ClassPathResource("com.xiaohuan.Test-schema.sql"),
				ClassPathResource("com.xiaohuan.Test-data.sql"))
		populator.setSeparator("@@")
...
Specifically, you can declare the `@Sql` annotation on a com.xiaohuan.Test class or com.xiaohuan.Test method to
configure individual SQL statements or the resource paths to SQL scripts that should be
run against a given database before or after an integration com.xiaohuan.Test method. Support for
...
of Spring Framework 5.2, however, this behavior may be configured per com.xiaohuan.Test class or per
com.xiaohuan.Test method via `@SqlMergeMode`. See
...
which the com.xiaohuan.Test class is defined. A path starting with a slash is treated as an absolute
...
within a JUnit Jupiter based integration com.xiaohuan.Test class:
...
----
	...
	class DatabaseTests {
		...
		void emptySchemaTest() {
			// execute code that uses the com.xiaohuan.Test schema without any com.xiaohuan.Test data
		}

		@com.xiaohuan.Test
		@Sql({"/com.xiaohuan.Test-schema.sql", "/com.xiaohuan.Test-user-data.sql"})
		void userTest() {
			// execute code that uses the com.xiaohuan.Test schema and com.xiaohuan.Test data
		}
	}
...
* Class-level declaration: If the annotated com.xiaohuan.Test class is `com.example.MyTest`, the
  corresponding default script is `classpath:com/example/MyTest.sql`.
* Method-level declaration: If the annotated com.xiaohuan.Test method is named `testMethod()` and is
...
If you need to configure multiple sets of SQL scripts for a given com.xiaohuan.Test class or com.xiaohuan.Test
...
In the scenario presented in the preceding example, the `com.xiaohuan.Test-schema.sql` script uses a
...
By default, SQL scripts are executed before the corresponding com.xiaohuan.Test method. However, if
you need to run a particular set of scripts after the com.xiaohuan.Test method (for example, to clean
...
----
	@com.xiaohuan.Test
	@Sql(
		scripts = "create-com.xiaohuan.Test-data.sql",
		config = @SqlConfig(transactionMode = ISOLATED)
	)
	@Sql(
		scripts = "delete-com.xiaohuan.Test-data.sql",
		config = @SqlConfig(transactionMode = ISOLATED),
		executionPhase = AFTER_TEST_METHOD
	)
	void userTest() {
		// execute code that needs the com.xiaohuan.Test data to be committed
		// to the database outside of the com.xiaohuan.Test's transaction
	}
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
.Kotlin
----
	@com.xiaohuan.Test
	@SqlGroup(
		Sql("create-com.xiaohuan.Test-data.sql",
			config = SqlConfig(transactionMode = ISOLATED)),
		Sql("delete-com.xiaohuan.Test-data.sql",
			config = SqlConfig(transactionMode = ISOLATED),
			executionPhase = AFTER_TEST_METHOD))
	fun userTest() {
		// execute code that needs the com.xiaohuan.Test data to be committed
...
When declared as a class-level annotation on an integration com.xiaohuan.Test class, `@SqlConfig`
serves as global configuration for all SQL scripts within the com.xiaohuan.Test class hierarchy. When
...
individual attributes in {api-spring-framework}/com.xiaohuan.Test/context/jdbc/Sql.html[`@Sql`] and
{api-spring-framework}/com.xiaohuan.Test/context/jdbc/SqlConfig.html[`@SqlConfig`] for details.
...
transaction managed by the `TransactionalTestExecutionListener` for a com.xiaohuan.Test annotated with
...
`PlatformTransactionManager` in the com.xiaohuan.Test's `ApplicationContext`. As a bare minimum,
however, a `javax.sql.DataSource` must be present in the com.xiaohuan.Test's `ApplicationContext`.
...
{api-spring-framework}/com.xiaohuan.Test/context/jdbc/SqlConfig.html[`@SqlConfig`] and
{api-spring-framework}/com.xiaohuan.Test/context/jdbc/SqlScriptsTestExecutionListener.html[`SqlScriptsTestExecutionListener`]
...
----
	...
	class TransactionalSqlScriptsTests {
		...
		@com.xiaohuan.Test
		@Sql("/com.xiaohuan.Test-data.sql")
		void usersTest() {
			// verify state in com.xiaohuan.Test database:
			assertNumUsers(2);
			// execute code that uses the com.xiaohuan.Test data...
		}
...
run, since any changes made to the database (either within the com.xiaohuan.Test method or within the
`/com.xiaohuan.Test-data.sql` script) are automatically rolled back by the
...
database schema or some common com.xiaohuan.Test data once per com.xiaohuan.Test class and then provide additional,
use case specific com.xiaohuan.Test data per com.xiaohuan.Test method. To enable `@Sql` merging, annotate either
your com.xiaohuan.Test class or com.xiaohuan.Test method with `@SqlMergeMode(MERGE)`. To disable merging for a
specific com.xiaohuan.Test method (or specific com.xiaohuan.Test subclass), you can switch back to the default mode
...
[[testcontext-parallel-com.xiaohuan.Test-execution]]
==== Parallel com.xiaohuan.Test Execution
...
com.xiaohuan.Test classes or com.xiaohuan.Test methods can be executed in parallel without any changes to com.xiaohuan.Test code
or configuration.

TIP: For details on how to set up parallel com.xiaohuan.Test execution, see the documentation for your
testing framework, build tool, or IDE.

Keep in mind that the introduction of concurrency into your com.xiaohuan.Test suite can result in
...
* Use JUnit 4's `@FixMethodOrder` support or any testing framework feature
  that is designed to ensure that com.xiaohuan.Test methods run in a particular order. Note,
  however, that this does not apply if entire com.xiaohuan.Test classes are executed in parallel.
...
[TIP]
====
If parallel com.xiaohuan.Test execution fails with an exception stating that the `ApplicationContext`
for the current com.xiaohuan.Test is no longer active, this typically means that the
...
WARNING: Parallel com.xiaohuan.Test execution in the Spring TestContext Framework is only possible if
the underlying `TestContext` implementation provides a copy constructor, as explained in
the javadoc for {api-spring-framework}/com.xiaohuan.Test/context/TestContext.html[`TestContext`]. The
...
verify that it is suitable for parallel com.xiaohuan.Test execution.
...
runner (supported on JUnit 4.12 or higher). By annotating com.xiaohuan.Test classes with
...
loading application contexts, dependency injection of com.xiaohuan.Test instances, transactional com.xiaohuan.Test
...
The following code listing shows the minimal requirements for configuring a com.xiaohuan.Test class to
...
----
	...
	public class SimpleTest {
		...
		public void testMethod() {
			// execute com.xiaohuan.Test logic...
		}
	}
----
...
The `org.springframework.com.xiaohuan.Test.context.junit4.rules` package provides the following JUnit
...
to declare these rules in an integration com.xiaohuan.Test:
...
The `org.springframework.com.xiaohuan.Test.context.junit4` package provides the following support
classes for JUnit 4-based com.xiaohuan.Test cases (supported on JUnit 4.12 or higher):

* `AbstractJUnit4SpringContextTests`
* `AbstractTransactionalJUnit4SpringContextTests`

`AbstractJUnit4SpringContextTests` is an abstract base com.xiaohuan.Test class that integrates the
...
bean lookups or to com.xiaohuan.Test the state of the context as a whole.
...
TIP: These classes are a convenience for extension. If you do not want your com.xiaohuan.Test classes
to be tied to a Spring-specific class hierarchy, you can configure your own custom com.xiaohuan.Test
...
framework, introduced in JUnit 5. By annotating com.xiaohuan.Test classes with
...
such as support for loading application contexts, dependency injection of com.xiaohuan.Test instances,
transactional com.xiaohuan.Test method execution, and so on.
...
* Dependency injection for com.xiaohuan.Test constructors, com.xiaohuan.Test methods, and com.xiaohuan.Test lifecycle callback
...
* Powerful support for link:https://junit.org/junit5/docs/current/user-guide/#extensions-conditions[conditional
  com.xiaohuan.Test execution] based on SpEL expressions, environment variables, system properties,
...
The following code listing shows how to configure a com.xiaohuan.Test class to use the
`SpringExtension` in conjunction with `@ContextConfiguration`:

[source,java,indent=0,subs="verbatim,quotes",role="primary"]
.Java
----
	// Instructs JUnit Jupiter to extend the com.xiaohuan.Test with Spring support.
	@ExtendWith(SpringExtension.class)
...
configuration of the com.xiaohuan.Test `ApplicationContext` and JUnit Jupiter.
...
extension API from JUnit Jupiter, which lets Spring provide dependency injection for com.xiaohuan.Test
constructors, com.xiaohuan.Test methods, and com.xiaohuan.Test lifecycle callback methods.

Specifically, `SpringExtension` can inject dependencies from the com.xiaohuan.Test's
`ApplicationContext` into com.xiaohuan.Test constructors and methods that are annotated with
`@BeforeAll`, `@AfterAll`, `@BeforeEach`, `@AfterEach`, `@com.xiaohuan.Test`, `@RepeatedTest`,
`@ParameterizedTest`, and others.

[[testcontext-junit-jupiter-di-constructor]]
====== Constructor Injection

If a specific parameter in a constructor for a JUnit Jupiter com.xiaohuan.Test class is of type
...
parameter with the corresponding bean or value from the com.xiaohuan.Test's `ApplicationContext`.

Spring can also be configured to autowire all arguments for a com.xiaohuan.Test class constructor if
...
* `@TestConstructor` is present or meta-present on the com.xiaohuan.Test class with the `autowireMode`
...
WARNING: If the constructor for a com.xiaohuan.Test class is considered to be _autowirable_, Spring
...
[WARNING]
====
Constructor injection for com.xiaohuan.Test classes must not be used in conjunction with JUnit
Jupiter's `@TestInstance(PER_CLASS)` support if `@DirtiesContext` is used to close the
com.xiaohuan.Test's `ApplicationContext` before or after com.xiaohuan.Test methods.

The reason is that `@TestInstance(PER_CLASS)` instructs JUnit Jupiter to cache the com.xiaohuan.Test
instance between com.xiaohuan.Test method invocations. Consequently, the com.xiaohuan.Test instance will retain
references to beans that were originally injected from an `ApplicationContext` that has
been subsequently closed. Since the constructor for the com.xiaohuan.Test class will only be invoked
...
To use `@DirtiesContext` with "before com.xiaohuan.Test method" or "after com.xiaohuan.Test method" modes in
conjunction with `@TestInstance(PER_CLASS)`, one must configure dependencies from Spring
to be supplied via field or setter injection so that they can be re-injected between com.xiaohuan.Test
method invocations.
====
...
Note that this feature lets com.xiaohuan.Test dependencies be `final` and therefore immutable.

If the `spring.com.xiaohuan.Test.constructor.autowire.mode` property is to `all` (see
...
If a parameter in a JUnit Jupiter com.xiaohuan.Test method or com.xiaohuan.Test lifecycle callback method is of
type `ApplicationContext` (or a sub-type thereof) or is annotated or meta-annotated with
`@Autowired`, `@Qualifier`, or `@Value`, Spring injects the value for that specific
parameter with the corresponding bean from the com.xiaohuan.Test's `ApplicationContext`.

In the following example, Spring injects the `OrderService` from the `ApplicationContext`
loaded from `TestConfig.class` into the `deleteOrder()` com.xiaohuan.Test method:
...
----
	...
	class OrderServiceIntegrationTests {
		...
		void deleteOrder(@Autowired OrderService orderService) {
			// use orderService from the com.xiaohuan.Test's ApplicationContext
		}
	}
----
...
into the `placeOrderRepeatedly()` com.xiaohuan.Test method simultaneously.
...
Note that the use of `@RepeatedTest` from JUnit Jupiter lets the com.xiaohuan.Test method gain access
to the `RepetitionInfo`.

[[testcontext-support-classes-testng]]
===== TestNG Support Classes

The `org.springframework.com.xiaohuan.Test.context.testng` package provides the following support
classes for TestNG based com.xiaohuan.Test cases:

* `AbstractTestNGSpringContextTests`
* `AbstractTransactionalTestNGSpringContextTests`

`AbstractTestNGSpringContextTests` is an abstract base com.xiaohuan.Test class that integrates the
Spring TestContext Framework with explicit `ApplicationContext` testing support in a
...
manually instrumenting your com.xiaohuan.Test class with a `TestContextManager`. See the source code
of `AbstractTestNGSpringContextTests` for an example of how to instrument your com.xiaohuan.Test class.
...
The Spring MVC com.xiaohuan.Test framework provides first class support for testing Spring MVC code
...
from the `spring-com.xiaohuan.Test` module and, hence, does not use a running Servlet container. It
...
addition to a standalone mode, in which you can manually instantiate controllers and com.xiaohuan.Test
them one at a time.

Spring MVC com.xiaohuan.Test also provides client-side support for testing code that uses the
...
integration tests, see <<spring-mvc-com.xiaohuan.Test-vs-end-to-end-integration-tests>>.


[[spring-mvc-com.xiaohuan.Test-server]]
==== Server-Side Tests

You can write a plain unit com.xiaohuan.Test for a Spring MVC controller by using JUnit or TestNG. To
...
others, as necessary). However, when writing such a unit com.xiaohuan.Test, much remains untested: for
...
The goal of Spring MVC com.xiaohuan.Test is to provide an effective way to com.xiaohuan.Test controllers by
performing requests and generating responses through the actual `DispatcherServlet`.

Spring MVC com.xiaohuan.Test builds on the familiar <<mock-objects-servlet, "`mock`" implementations of
the Servlet API>> available in the `spring-com.xiaohuan.Test` module. This allows performing requests
...
explained in <<spring-mvc-com.xiaohuan.Test-vs-end-to-end-integration-tests>>. The following JUnit
Jupiter-based example uses Spring MVC com.xiaohuan.Test:

[source,java,indent=0,subs="verbatim,quotes",role="primary"]
.Java
----
	import static org.springframework.com.xiaohuan.Test.web.servlet.request.MockMvcRequestBuilders.*;
	import static org.springframework.com.xiaohuan.Test.web.servlet.result.MockMvcResultMatchers.*;

	@SpringJUnitWebConfig(locations = "com.xiaohuan.Test-servlet-context.xml")
	class ExampleTests {
...
.Kotlin
----
	import org.springframework.com.xiaohuan.Test.web.servlet.get
...
The preceding com.xiaohuan.Test relies on the `WebApplicationContext` support of the TestContext
framework to load Spring configuration from an XML configuration file located in the same
package as the com.xiaohuan.Test class, but Java-based and Groovy-based configuration are also
supported. See these
https://github.com/spring-projects/spring-framework/tree/master/spring-com.xiaohuan.Test/src/com.xiaohuan.Test/java/org/springframework/com.xiaohuan.Test/web/servlet/samples/context[sample tests].
...
[[spring-mvc-com.xiaohuan.Test-server-static-imports]]
===== Static Imports

The fluent API in the example from the <<spring-mvc-com.xiaohuan.Test-server, preceding section>>
...
[[spring-mvc-com.xiaohuan.Test-server-setup-options]]
===== Setup Choices
...
configuration and injects a `WebApplicationContext` into the com.xiaohuan.Test to use to build a
...
complete integration com.xiaohuan.Test. Since the TestContext framework caches the loaded Spring
configuration, it helps keep tests running fast, even as you introduce more tests in your
com.xiaohuan.Test suite. Furthermore, you can inject mock services into controllers through Spring
...
You can then inject the mock service into the com.xiaohuan.Test to set up and verify your
...
The `standaloneSetup`, on the other hand, is a little closer to a unit com.xiaohuan.Test. It tests one
...
com.xiaohuan.Test against your actual Spring MVC configuration.

[[spring-mvc-com.xiaohuan.Test-server-setup-steps]]
===== Setup Features
...
See the javadoc for
{api-spring-framework}/com.xiaohuan.Test/web/servlet/setup/ConfigurableMockMvcBuilder.html[`ConfigurableMockMvcBuilder`]
for a list of all MockMvc builder features or use the IDE to explore the available options.

[[spring-mvc-com.xiaohuan.Test-server-performing-requests]]
===== Performing Requests
...
.Kotlin
----
	import org.springframework.com.xiaohuan.Test.web.servlet.post
...
.Kotlin
----
	import org.springframework.com.xiaohuan.Test.web.servlet.multipart
...
request URI. If you must com.xiaohuan.Test with the full request URI, be sure to set the `contextPath`
...
[[spring-mvc-com.xiaohuan.Test-server-defining-expectations]]
...
The following com.xiaohuan.Test asserts that binding or validation failed:
...
`org.springframework.com.xiaohuan.Test.web.servlet.result` logging category.
...
[[spring-mvc-com.xiaohuan.Test-async-requests]]
===== Async Requests
...
In Spring MVC com.xiaohuan.Test, async requests can be tested by asserting the produced async value
first, then manually performing the async dispatch, and finally verifying the response.
Below is an example com.xiaohuan.Test for controller methods that return `DeferredResult`, `Callable`,
...
----
	@com.xiaohuan.Test
	void com.xiaohuan.Test() throws Exception {
...
[[spring-mvc-com.xiaohuan.Test-vs-streaming-response]]
===== Streaming Responses

There are no options built into Spring MVC com.xiaohuan.Test for container-less testing of streaming
...
{doc-spring-boot}/html/spring-boot-features.html#boot-features-testing-spring-boot-applications-testing-with-running-server[com.xiaohuan.Test a running server]
...
[[spring-mvc-com.xiaohuan.Test-server-filters]]
...
Registered filters are invoked through the `MockFilterChain` from `spring-com.xiaohuan.Test`, and the
last filter delegates to the `DispatcherServlet`.


[[spring-mvc-com.xiaohuan.Test-vs-end-to-end-integration-tests]]
===== Spring MVC com.xiaohuan.Test vs End-to-End Tests

Spring MVC com.xiaohuan.Test is built on Servlet API mock implementations from the
`spring-com.xiaohuan.Test` module and does not rely on a running container. Therefore, there are
...
There are pros and cons for each approach. The options provided in Spring MVC com.xiaohuan.Test are
different stops on the scale from classic unit testing to full integration testing. To be
certain, none of the options in Spring MVC com.xiaohuan.Test fall under the category of classic unit
...
might com.xiaohuan.Test the data access layer in isolation from the layers above it. Also, you can use
...
Another important distinction when using Spring MVC com.xiaohuan.Test is that, conceptually, such
...
[[spring-mvc-com.xiaohuan.Test-server-resources]]
===== Further Examples

The framework's own tests include
https://github.com/spring-projects/spring-framework/tree/master/spring-com.xiaohuan.Test/src/com.xiaohuan.Test/java/org/springframework/com.xiaohuan.Test/web/servlet/samples[many
sample tests] intended to show how to use Spring MVC com.xiaohuan.Test. You can browse these examples
for further ideas. Also, the
https://github.com/spring-projects/spring-mvc-showcase[`spring-mvc-showcase`] project has
full com.xiaohuan.Test coverage based on Spring MVC com.xiaohuan.Test.


[[spring-mvc-com.xiaohuan.Test-server-htmlunit]]
==== HtmlUnit Integration

Spring provides integration between <<spring-mvc-com.xiaohuan.Test-server, MockMvc>> and
...
* Easily com.xiaohuan.Test HTML pages by using tools such as
...
* com.xiaohuan.Test JavaScript within pages.
* Optionally, com.xiaohuan.Test using mock services to speed up testing.
...
[[spring-mvc-com.xiaohuan.Test-server-htmlunit-why]]
...
With Spring MVC com.xiaohuan.Test, we can easily com.xiaohuan.Test if we are able to create a `Message`, as follows:
...
What if we want to com.xiaohuan.Test the form view that lets us create the message? For example,
...
This com.xiaohuan.Test has some obvious drawbacks. If we update our controller to use the parameter
`message` instead of `text`, our form com.xiaohuan.Test continues to pass, even though the HTML form
is out of synch with the controller. To resolve this we can combine our two tests, as
follows:

[[spring-mvc-com.xiaohuan.Test-server-htmlunit-mock-mvc-com.xiaohuan.Test]]
[source,java,indent=0,subs="verbatim,quotes",role="primary"]
.Java
----
...
This would reduce the risk of our com.xiaohuan.Test incorrectly passing, but there are still some
problems:
...
* Finally, we still cannot account for some things. For example, what if the form has
  JavaScript validation that we wish to com.xiaohuan.Test as well?
...
[[spring-mvc-com.xiaohuan.Test-server-htmlunit-why-integration]]
...
* Ensuring the proper messages are in the database can be tedious. (Consider foreign key
  constraints.)
* Testing can become slow, since each com.xiaohuan.Test would need to ensure that the database is in
  the correct state.
...
[[spring-mvc-com.xiaohuan.Test-server-htmlunit-why-mockmvc]]
====== Enter HtmlUnit Integration

So how can we achieve a balance between testing the interactions of our pages and still
retain good performance within our com.xiaohuan.Test suite? The answer is: "`By integrating MockMvc
with HtmlUnit.`"

[[spring-mvc-com.xiaohuan.Test-server-htmlunit-options]]
====== HtmlUnit Integration Options

You have a number of options when you want to integrate MockMvc with HtmlUnit:

* <<spring-mvc-com.xiaohuan.Test-server-htmlunit-mah,MockMvc and HtmlUnit>>: Use this option if you
  want to use the raw HtmlUnit libraries.
* <<spring-mvc-com.xiaohuan.Test-server-htmlunit-webdriver,MockMvc and WebDriver>>: Use this option to
  ease development and reuse code between integration and end-to-end testing.
* <<spring-mvc-com.xiaohuan.Test-server-htmlunit-geb,MockMvc and Geb>>: Use this option if you want to
  use Groovy for testing, ease development, and reuse code between integration and
  end-to-end testing.

[[spring-mvc-com.xiaohuan.Test-server-htmlunit-mah]]
...
[[spring-mvc-com.xiaohuan.Test-server-htmlunit-mah-setup]]
====== MockMvc and HtmlUnit Setup

First, make sure that you have included a com.xiaohuan.Test dependency on
...
see <<spring-mvc-com.xiaohuan.Test-server-htmlunit-mah-advanced-builder>>.
...
requested by using a network connection, as normal. This lets us easily com.xiaohuan.Test the use of
CDNs.

[[spring-mvc-com.xiaohuan.Test-server-htmlunit-mah-usage]]
...
as described in <<spring-mvc-com.xiaohuan.Test-server-htmlunit-mah-advanced-builder>>.
...
<<spring-mvc-com.xiaohuan.Test-server-htmlunit-mock-mvc-com.xiaohuan.Test, MockMvc com.xiaohuan.Test>> in a number of ways.
...
uses the Mozilla Rhino engine] to evaluate JavaScript. This means that we can also com.xiaohuan.Test
...
[[spring-mvc-com.xiaohuan.Test-server-htmlunit-mah-advanced-builder]]
...
[[spring-mvc-com.xiaohuan.Test-server-htmlunit-webdriver]]
...
[[spring-mvc-com.xiaohuan.Test-server-htmlunit-webdriver-why]]
...
This approach results in numerous separate tests because we want to com.xiaohuan.Test error conditions
...
[[spring-mvc-com.xiaohuan.Test-server-htmlunit-webdriver-setup]]
====== MockMvc and WebDriver Setup

To use Selenium WebDriver with the Spring MVC com.xiaohuan.Test framework, make sure that your project
includes a com.xiaohuan.Test dependency on `org.seleniumhq.selenium:selenium-htmlunit-driver`.
...
usage, see <<spring-mvc-com.xiaohuan.Test-server-htmlunit-webdriver-advanced-builder>>.
...
URL is requested by using a network connection, as normal. This lets us easily com.xiaohuan.Test the
use of CDNs.

[[spring-mvc-com.xiaohuan.Test-server-htmlunit-webdriver-usage]]
...
This improves on the design of our <<spring-mvc-com.xiaohuan.Test-server-htmlunit-mah-usage, HtmlUnit com.xiaohuan.Test>>
by leveraging the Page Object Pattern. As we mentioned in
<<spring-mvc-com.xiaohuan.Test-server-htmlunit-webdriver-why>>, we can use the Page Object Pattern
...
Lastly, we must not forget to close the `WebDriver` instance when the com.xiaohuan.Test is complete,
as follows:
...
[[spring-mvc-com.xiaohuan.Test-server-htmlunit-webdriver-advanced-builder]]
...
[[spring-mvc-com.xiaohuan.Test-server-htmlunit-geb]]
===== MockMvc and Geb
...
[[spring-mvc-com.xiaohuan.Test-server-htmlunit-geb-why]]
====== Why Geb and MockMvc?

Geb is backed by WebDriver, so it offers many of the
<<spring-mvc-com.xiaohuan.Test-server-htmlunit-webdriver-why, same benefits>> that we get from
WebDriver. However, Geb makes things even easier by taking care of some of the
boilerplate code for us.

[[spring-mvc-com.xiaohuan.Test-server-htmlunit-geb-setup]]
...
[[spring-mvc-com.xiaohuan.Test-server-htmlunit-geb-usage]]
...
<<spring-mvc-com.xiaohuan.Test-server-htmlunit-mah-usage, HtmlUnit com.xiaohuan.Test>> by using the Page Object
...
[[spring-mvc-com.xiaohuan.Test-client]]
==== Client-Side REST Tests

You can use client-side tests to com.xiaohuan.Test code that internally uses the `RestTemplate`. The
...
----
	...
	// com.xiaohuan.Test code that uses the above RestTemplate ...
...
As an alternative to all of the above, the client-side com.xiaohuan.Test support also provides a
...
[[spring-mvc-com.xiaohuan.Test-client-static-imports]]
===== Static Imports
...
[[spring-mvc-com.xiaohuan.Test-client-resources]]
===== Further Examples of Client-side REST Tests

Spring MVC com.xiaohuan.Test's own tests include
https://github.com/spring-projects/spring-framework/tree/master/spring-com.xiaohuan.Test/src/com.xiaohuan.Test/java/org/springframework/com.xiaohuan.Test/web/client/samples[example
...
* https://www.junit.org/[JUnit]: "`A programmer-friendly testing framework for Java`".
  Used by the Spring Framework in its com.xiaohuan.Test suite and supported in the
...
* https://testng.org/[TestNG]: A testing framework inspired by JUnit with added support
  for com.xiaohuan.Test groups, data-driven testing, distributed testing, and other features. Supported
...
* http://www.mockobjects.com/[MockObjects.com]: Web site dedicated to mock objects, a
  technique for improving the design of code within com.xiaohuan.Test-driven development.
* https://mockito.github.io[Mockito]: Java mock library based on the
  http://xunitpatterns.com/com.xiaohuan.Test%20Spy.html[com.xiaohuan.Test Spy] pattern. Used by the Spring Framework
  in its com.xiaohuan.Test suite.
...
* https://jmock.org/[JMock]: Library that supports com.xiaohuan.Test-driven development of Java code
  with mock objects.
* https://www.dbunit.org/[DbUnit]: JUnit extension (also usable with Ant and Maven) that
  ...
  a known state between com.xiaohuan.Test runs.
