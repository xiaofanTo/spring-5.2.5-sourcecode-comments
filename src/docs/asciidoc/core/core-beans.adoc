...
to com.xiaohuan.Test, particularly when the dependencies are on interfaces or abstract base classes,
...
After you write and com.xiaohuan.Test one or more custom `Scope` implementations, you need to make
...
include it within `system-com.xiaohuan.Test-config.xml` as a `<bean/>` definition. Because
...
The following example shows part of a sample `system-com.xiaohuan.Test-config.xml` file:
...
----
	public static void main(String[] args) {
		ApplicationContext ctx = new ClassPathXmlApplicationContext("classpath:/com/acme/system-com.xiaohuan.Test-config.xml");
		TransferService transferService = ctx.getBean(TransferService.class);
		// ...
	}
----
...
----
	fun main() {
		val ctx = ClassPathXmlApplicationContext("classpath:/com/acme/system-com.xiaohuan.Test-config.xml")
		val transferService = ctx.getBean<TransferService>()
		// ...
	}
----
...
NOTE: In `system-com.xiaohuan.Test-config.xml` file, the `AppConfig` `<bean/>` does not declare an `id`
...
describe in the previous example, we can redefine `system-com.xiaohuan.Test-config.xml` to take advantage of component-scanning.
...
The following example shows the modified `system-com.xiaohuan.Test-config.xml` file:
...
`DataSource`. In a com.xiaohuan.Test environment, the configuration might resemble the following:
...
----
	...
	public DataSource dataSource() {
		return new EmbeddedDatabaseBuilder()
			...
			.addScript("my-com.xiaohuan.Test-data.sql")
			.build();
	}
----
[source,kotlin,indent=0,subs="verbatim,quotes",role="secondary"]
...
----
	<beans profile="development"
		...
		<jdbc:embedded-database id="dataSource">
			...
			<jdbc:script location="classpath:com/bank/config/sql/com.xiaohuan.Test-data.sql"/>
...
profiles can be declared by using the `@ActiveProfiles` annotation in the `spring-com.xiaohuan.Test`
