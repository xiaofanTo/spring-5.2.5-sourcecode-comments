...
{api-spring-framework}/com.xiaohuan.Test/context/TestConstructor.html[`@TestConstructor(autowireMode = AutowireMode.ALL)`]
...
Kotlin lets you specify meaningful com.xiaohuan.Test function names between backticks (```).
As of JUnit 5, Kotlin com.xiaohuan.Test classes can use the `@TestInstance(TestInstance.Lifecycle.PER_CLASS)`
annotation to enable a single instantiation of com.xiaohuan.Test classes, which allows the use of `@BeforeAll`
