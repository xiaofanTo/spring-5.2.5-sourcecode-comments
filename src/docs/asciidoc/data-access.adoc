...
transactions a worthwhile abstraction, even when you work with JTA. You can com.xiaohuan.Test transactional code
...
that com.xiaohuan.Test drives the configuration shown earlier:
...
----
	...
	<beans xmlns="http://www.springframework.org/schema/beans"
		...
		<context:component-scan base-package="org.springframework.docs.com.xiaohuan.Test" />
...
the datasource. You most likely fill both roles as you develop and com.xiaohuan.Test code, but you
...
`SingleConnectionDataSource` is primarily a com.xiaohuan.Test class. For example, it enables easy testing of code outside an
...
This implementation is useful for com.xiaohuan.Test and stand-alone environments outside of a Java EE
...
using JavaBean-style connection pools (such as `commons-dbcp`) is so easy, even in a com.xiaohuan.Test
...
.Java
----
	final TestItem testItem = new TestItem(123L, "A com.xiaohuan.Test item",
...
.Kotlin
----
	val (id, description, expirationDate) = TestItem(123L, "A com.xiaohuan.Test item",
...
----
	<jdbc:embedded-database id="dataSource" generate-name="true">
		...
		<jdbc:script location="classpath:com.xiaohuan.Test-data.sql"/>
	</jdbc:embedded-database>
----

The preceding configuration creates an embedded HSQL database that is populated with SQL from
the `schema.sql` and `com.xiaohuan.Test-data.sql` resources in the root of the classpath. In addition, as
...
stand-alone environment or in a stand-alone integration com.xiaohuan.Test, as in the following example:
...
Embedded databases provide a lightweight way to com.xiaohuan.Test data access code. The next example is a
data access integration com.xiaohuan.Test template that uses an embedded database. Using such a template
can be useful for one-offs when the embedded database does not need to be reused across com.xiaohuan.Test
classes. However, if you wish to create an embedded database that is shared within a com.xiaohuan.Test suite,
...
shows the com.xiaohuan.Test template:
...
Development teams often encounter errors with embedded databases if their com.xiaohuan.Test suite
...
across multiple testing scenarios within the same com.xiaohuan.Test suite (that is, within the same JVM
...
----
	<jdbc:initialize-database data-source="dataSource">
		...
		<jdbc:script location="classpath:com/foo/sql/db-com.xiaohuan.Test-data.sql"/>
	</jdbc:initialize-database>
----

The preceding example runs the two specified scripts against the database. The first
script creates a schema, and the second populates tables with a com.xiaohuan.Test data set. The script
...
the scripts against a database that already has com.xiaohuan.Test data in it. The likelihood
...
EXISTS` (or similar) but you want to unconditionally remove all com.xiaohuan.Test data before
...
----
	<jdbc:initialize-database data-source="dataSource" separator="@@"> <1>
		...
		<jdbc:script location="classpath:com/myapp/sql/db-com.xiaohuan.Test-data-1.sql"/>
		<jdbc:script location="classpath:com/myapp/sql/db-com.xiaohuan.Test-data-2.sql"/>
	</jdbc:initialize-database>
----
<1> Set the separator scripts to `@@`.
<2> Set the separator for `db-schema.sql` to `;`.

In this example, the two `com.xiaohuan.Test-data` scripts use `@@` as statement separator and only
...
* *Easier testing.* Spring's IoC approach makes it easy to swap the implementations
  ...
  in turn makes it much easier to com.xiaohuan.Test each piece of persistence-related code in
  isolation.
	...
	public class ProductDaoImpl implements ProductDao {
		...
		public Collection loadProductsByCategory(String category) {
			return this.sessionFactory.getCurrentSession()
					.createQuery("from com.xiaohuan.Test.Product product where product.category=?")
...
other than JTA, transaction support also works in a stand-alone or com.xiaohuan.Test environment.
...
tier in desktop applications or com.xiaohuan.Test suites.
...
option is sufficient only for stand-alone applications and com.xiaohuan.Test environments, for which
